<script lang="ts">
  ...
  $effect(() => {
    const unsub = productsStore.subscribe((s) => {
      products = s.items;
      loadingProducts = s.loading;
      productsError = s.error;
    });
  ...
  // GSAP grid animation (re-run on visibleProducts changes)
  $effect(() => {
    const currentProducts = visibleProducts;
...
<section class="min-h-screen bg-[#0a0a0a] py-32 px-6 md:px-12">
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-8">
      <div class="w-full md:w-auto">
        ...
        <div class="mt-8 relative max-w-md group">
          ...
          />
          <svg
            class="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 group-focus-within:text-white transition-colors duration-300"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        {#if loadingProducts}
          <p class="mt-6 text-white/40 tracking-[0.2em] uppercase text-xs">
            Loading products...
          </p>
  ...
  >
    {#each visibleProducts as product, index (product.id)}
      <ProductCard {product} {index} onQuickView={handleQuickView} />
    {:else}
      <div class="col-span-full py-20 text-center">
        <p class="text-white/40 tracking-[0.2em] uppercase text-xs">
